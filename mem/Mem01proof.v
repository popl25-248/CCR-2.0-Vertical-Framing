...
Section SIMMODSEM.
  ...
  Theorem correct_modsem: forall sk, ModSemPair.sim (SModSem.to_tgt MemConds
  ...
  Proof.
   ...
   { ss. }
    ...
    econs; ss.
    { unfold allocF. init.
      (*** TODO: question: is there some tactic like "eassert" that allows us to avoid writing "alloc_spec" manually? ***)
      assert(SPC: (MemConds "alloc") = alloc_spec).
      { unfold MemConds, MemStb. autorewrite with stb. ss. }
      rewrite ! SPC in *.
    ...
    econs; ss.
    { unfold freeF. init.
      assert(SPC: (MemConds "free") = free_spec).
      { unfold MemConds, MemStb. autorewrite with stb. ss. }
      rewrite ! SPC in *.
      ...
      { des_ifs_safe (mDesAll; ss). des; subst.
        ...
        rename a1 into v. rename WF into SIMWF.
    ...
    econs; ss.
    { unfold loadF. init.
      assert(SPC: (MemConds "load") = load_spec).
      { unfold MemConds, MemStb. autorewrite with stb. ss. }
      rewrite ! SPC in *.
      harg. fold wf. steps. hide_k.
      { des_ifs_safe (mDesAll; ss). mDesAll; ss. des; subst. clarify. rewrite Any.upcast_downcast in *. clarify.
        steps. unhide_k. steps.
        renamer. rename n into b. rename z into ofs.
    ...
    econs; ss.
    { unfold storeF. init.
      assert(SPC: (MemConds "store") = store_spec).
      { unfold MemConds, MemStb. autorewrite with stb. ss. }
      rewrite ! SPC in *.
      harg. fold wf. steps. hide_k.
      { des_ifs_safe (mDesAll; ss). mDesAll; ss. des; subst. clarify. rewrite Any.upcast_downcast in *. clarify.
        ...
        rename a1 into v0. rename WF into SIMWF.
        steps.
        mCombine "INV" "A". mOwnWf "INV".
    ...
    econs; ss.
    { unfold cmpF. init.
      assert(SPC: (MemConds "cmp") = cmp_spec).
      { unfold MemConds, MemStb. autorewrite with stb. ss. }
      rewrite ! SPC in *.
      harg. fold wf. steps. hide_k.
      { des_ifs_safe (mDesAll; ss). mDesAll; ss. des; subst. clarify.
        steps. unhide_k. steps.
        renamer.
