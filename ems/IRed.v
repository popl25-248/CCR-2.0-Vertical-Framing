...
Section TEST.
  ...
  Hypothesis xx_ext: forall R p q (i0 i1: itree _ R), i0 = i1 -> (xx p i0 q) = (xx p i1 q).
...
Section TEST2.
  (*** A regression test for interp_condE_tgt_rdb. ***)
  Hypothesis BACKDOOR: False.
  Class myClass: Type := { myTy :> Type }.
  Section DECLARATION.
    Context `{Σ: myClass}.
    Context `{eventE -< E}.
    Definition yy: forall T, myTy -> itree E T -> nat -> itree E T.
      revert H.
      destruct BACKDOOR.
    Defined.
  End DECLARATION.
  ...
  Section DECLARATION.
    Context `{Σ: myClass}.
    Context `{eventE -< E}.
    Theorem yy_bind: forall R S i (k: ktree _ R S) p q, (yy p (i >>= k) q) = (r <- yy p i q;; yy p (k r) q).
      destruct BACKDOOR. Defined.
    Theorem yy_tau: forall R p q (i: itree _ R), (yy p (tau;; i) q) = tau;; (yy p i q).
      destruct BACKDOOR. Defined.
    Theorem yy_ret: forall R p q (i: R), (yy p (Ret i) q) = Ret i.
      destruct BACKDOOR. Defined.
    Theorem yy_triggere: forall R p q (i: eventE R), (yy p (trigger i) q) = (trigger i >>= (fun r => tau;; Ret r)).
      destruct BACKDOOR. Defined.
    Theorem yy_UB: forall R p q, (yy p triggerUB q) = (triggerUB: itree _ R).
      destruct BACKDOOR. Defined.
    Theorem yy_NB: forall R p q, (yy p triggerNB q) = (triggerNB: itree _ R).
      destruct BACKDOOR. Defined.
    Theorem yy_unwrapU: forall R p q (i: option R), (yy p (unwrapU i) q) = (unwrapU i).
      destruct BACKDOOR. Defined.
    Theorem yy_unwrapN: forall R p q (i: option R), (yy p (unwrapN i) q) = (unwrapN i).
      destruct BACKDOOR. Defined.
    Theorem yy_assume: forall P p q, (yy p (assume P) q) = assume P >>= (fun _ => tau;; Ret tt).
      destruct BACKDOOR. Defined.
    Theorem yy_guarantee: forall P p q, (yy p (guarantee P) q) = guarantee P >>= (fun _ => tau;; Ret tt).
      destruct BACKDOOR. Defined.
    Theorem yy_ext: forall R p q (i0 i1: itree _ R), i0 = i1 -> (yy p i0 q) = (yy p i1 q).
      destruct BACKDOOR. Defined.
    (*** @yy does not work ***)
    Global Program Instance yy_rdb: red_database (mk_box (yy)) :=
      mk_rdb
        1
        (mk_box (yy_bind))
        (mk_box (yy_tau))
  ...
  (*** declaring outside does not work ***)
  (* Global Program Instance yy_rdb2 `{myClass, eventE -< E}: red_database (mk_box (@yy)) := *)
  (*   mk_rdb *)
  (*     1 *)
  (*     (mk_box (@yy_bind)) *)
  (*     (mk_box (@yy_tau)) *)
  ...
  Goal forall `{COND1: myClass} `{COND2: eventE -< E}
              p q T U V (i: itree E T) (j: ktree E T U) (k: ktree E U V),
      yy p (i >>= j >>= k) q = iret <- yy p i q;; jret <- yy p (j iret) q;; yy p (k jret) q
  .
  Proof.
