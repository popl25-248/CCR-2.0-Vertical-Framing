...
Section SIM.
  ...
  | sim_itree_call_tgt
      ...
      (* (FINDT: In (fn, ft) mt) *)
      (FINDT: alist_find fn mt = Some ft)
  ...
  Proof.
    ...
    { econsr; eauto. i. eapply sim_itree_mon; eauto. i. eapply rclo8_base. eauto. }
  Qed.
  ...
  Proof.
    ...
    { guclo sim_itree_indC_spec. econs 14; eauto. des. esplits; eauto. gbase. eauto. }
  Qed.
  ...
  Proof.
    ...
    { guclo tauNC_spec. econs; et. 2: { instantiate (2:=0). ss. } guclo sim_itree_indC_spec. econs; et. eapply IHSIM; et. f_equiv. ii. eapply TL. }
  Qed.
...
Lemma self_sim_itree_aux:
  forall {R} st (itr : itree Es R) mt f_src f_tgt,
    paco8 (_sim_itree mt (fun _ '(src, tgt) => src = tgt) top2) bot8 R R
      (fun x y z w => x = y /\ z = w) f_src f_tgt tt (st, itr) (st, itr).
Proof.
...
Proof.
  i. r.
  replace (lift_rel (fun (_ : ()) '(src, tgt) => src = tgt) top2 () eq) with
    (fun (x y: Any.t) (z w: Any.t) => x = y /\ z = w).
  { eapply self_sim_itree_aux. }
  extensionalities x y z w. eapply prop_ext. split; i; des; ss. r in H. des; ss.
Qed.
...
Section SIMMODSEM.
  ...
  Inductive sim: Prop := mk {
    ...
    sim_fnsems: Forall2 (sim_fnsem ms_tgt.(ModSem.fnsems) wf le) ms_src.(ModSem.fnsems) ms_tgt.(ModSem.fnsems);
...
Section ADEQUACY.
  Section SEMPAIR.
    ...
    Hypothesis IN_IN: forall fn ft (IN: alist_find fn mt = Some ft),
        alist_find fn ms_tgt.(ModSemL.fnsems) = Some (fun arg => (transl_all mn (ft arg))).
    ...
    Hypothesis fnsems_find_iff:
      forall fn,
        ...
        (exists (f_src f_tgt: _ -> itree _ _),
            ...
            (<<SIM: sim_fsem mt wf le f_src f_tgt>>)).
    ...
    | my_r0_intro
        ...
        (SIM: sim_itree mt wf le o_src o_tgt w0 (st_src mn, itr_src) (st_tgt mn, itr_tgt))
    ...
    Proof.
      ...
      { destruct H.
        ...
        eapply (@sim_itree_ind mt world wf le Any.t Any.t (lift_rel wf le w0 (@eq Any.t)) P); subst P; ss; i; clarify.
        ...
        - hexploit (fnsems_find_iff fn). i. des.
          ...
          { hexploit (SIM (varg) (varg)); et. i. des.
            steps. rewrite SRC. rewrite TGT. unfold unwrapU. ired_both.
            apply simg_progress_flag.
            guclo bindC_spec. econs.
        ...
        - eapply simg_progress_flag. gbase. eapply CIH.
          left. econs; eauto.
        - eapply IN_IN in FINDT. steps. rewrite FINDT.
          cbn. hexploit IH; et. intro T. ired_both.
          guclo euttC_spec. econs; et.
          { refl. }
          { ired_both. f_equiv. ii. grind. ired_both. rewrite tau_euttge.
            ired_both. refl. }
      }
      { destruct H. ides itr.
        { steps. }
    ...
    Proof.
      ...
      { inv H. hexploit (SIM (initial_arg) (initial_arg)); et. i. des.
  ...
  Proof.
    ...
    eapply adequacy_local_aux in PR; et; cycle 1.
    ...
    }
    {
      ...
      { folder.
        des_ifs.
        { exfalso.
          clear - IN Heq FNWF SKEQ sim_fnsems.
          rewrite map_app in *.
          (* eapply NoDup_map_inv in FNWF. *)
          eapply NoDup_app_disjoint in FNWF; ss.
          { rewrite in_map_iff. esplits; et.
            eapply alist_find_some; et. rewrite <- SKEQ. et. }
          { ss. rewrite List.map_map. unfold map_snd. ss.
            assert(In fn (List.map fst (ModSem.fnsems (Mod.get_modsem md_src (Sk.sort sk_src))))).
            { clear - sim_fnsems IN SKEQ.
              eapply alist_find_some in IN.
              unfold sim_fnsem in *. unfold RelProd in *.
              unfold relation_conjunction in *. cbn in sim_fnsems.
              eapply (in_map fst) in IN. ss.
              eapply Forall2_apply_Forall2 in sim_fnsems.
              { eapply Forall2_In_r in sim_fnsems; et. instantiate (2:=fst) in sim_fnsems.
                instantiate (1:=eq) in sim_fnsems. des; subst. congruence. }
              ii. des. rr in H. ss.
            }
            rewrite in_map_iff in H. des. subst. destruct x; ss.
            rewrite in_map_iff. eexists (_, _). ss. esplits; et.
          }
        }
        unfold map_snd. rewrite alist_find_map. uo. rewrite IN.
        rewrite sim_mn. ss.
      }
    }
  Qed.
